...
export class UserDashboardComponent {
  ...
  async createNewGroupMeeting() {
    if (this.groupList().length < 1) {
      await this.feedbackService.openInfoDialog("No Group Selected",
        "There are no groups or none have been selected")
      return
    }

    const resultGroup: ILookupBase = await this.feedbackService.openSelectionDialog("Create Group Meeting",
