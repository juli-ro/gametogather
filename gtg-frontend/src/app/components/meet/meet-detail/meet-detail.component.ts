...
export class MeetDetailComponent {
  ...
  meetingName: FormControl<string | null> = new FormControl("")
  ...
  viewDateVotes: Signal<{ date: IMeetDateSuggestion; votes: IMeetUserVote[] }[]> = computed(() => {
    ...
    return currentMeeting?.meetDateSuggestions.map(date => ({
      date,
  ...
  async setSingleDate(date: Date) {
    ...
    if (meeting?.meetDateSuggestions.some(x =>
      toFullUtCDate(new Date(x.date)).getTime() === utCDate.getTime()
    )) {
  ...
  async selectDate(date: IMeetDateSuggestion) {
    if (this.meeting()?.meetDateSuggestions
        .some(x => x.isChosenDate) &&
      !date.isChosenDate) {
